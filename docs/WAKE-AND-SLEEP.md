...
A failed heartbeat or process setup also triggers subprocess cleanup. If publishing the game PID fails, the supervisor stops the game while retaining the data lock; if stopping fails, it keeps the process reference and lock for another cleanup attempt. A failed output-thread start does not skip the subsequent Wine shutdown checks.
