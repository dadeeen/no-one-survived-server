...
Bei ausgeschöpftem `MAX_CRASH_RESTARTS` oder `RESTART_ON_CRASH=false` bleibt der Supervisor im Zustand `ERROR` aktiv und der Healthcheck schlägt fehl. UDP und die Docker-Neustartrichtlinie starten das Spiel dann nicht erneut. `nosctl wake` oder `SIGUSR1` erlaubt einen neuen Start mit zurückgesetztem Crash-Zähler. Ein bewusst ausgelöster Containerneustart setzt den Zähler ebenfalls zurück.
